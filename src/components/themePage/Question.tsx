...
const Question: React.FC<QuestionProps> = ({ question }) => {
    const { addToFav, removeFromFav, favoriteQuestions, fetchFavorites } =
        useFavorites()
    ...
    useEffect(() => {
        setIsFavorite(
            favoriteQuestions.some(
                (favQuestion) => favQuestion.id === question.id,
            ),
        )
            ...
            <li key={question.id} className="w-full">
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <p className="flex-grow transform transition-transform duration-200 hover:scale-105">
                            {question.question}
                        </p>
                    </div>
                    <div className="w-1/4 flex items-center justify-between">
                        <button className="flex items-center gap-2 p-2 rounded-md transform transition-transform duration-200 hover:scale-105  ">
