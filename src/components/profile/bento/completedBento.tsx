...
interface CompletedBentoProps {
    questions: Question[]
}

const CompletedBento: React.FC<CompletedBentoProps> = ({ questions }) => {
    return (
                    ...
                    {questions && questions.length > 0 ? (
                        questions.slice(0, 6).map((question) => (
                            <li key={question.id} className="w-full">
                                <button className="relative flex items-center justify-between bg-[#2C2C30] rounded-md w-full h-[50px] px-4 transform transition-transform duration-200 hover:scale-105 focus:outline-none">
